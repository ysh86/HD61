-------------------------------------------------------------------------
名称      HD61700 CROSS ASSEMBLER for Win32 (Rev 0.33)
...
■ 機能
 PB-1000内蔵のアセンブラとほぼ同一(上位互換)ですが、以下の違いがあります。
  ...
  3.CASIO未公開の命令を数多くサポート。(24,25,2C,2D,50,C0～CF,E0～EF他)
    ニモニックは｢AI-アセンブラ形式｣と｢KC形式｣のどちらも利用できます。(混在も可)
  4.セカンドオペレーション拡張($0,$30,$31,LD&JR)等がディフォルトで指定可。
    (/n オプション指定にてOFF）
...
■ ファイル内容
  ...
  PB-1000.S    （PB-1000内蔵アセンブラにて利用可能な全命令サンプル）
...
■ オプション（省略可）
 アセンブル時に以下のオプションが指定できます。
  /p ---  PBF形式で出力する。（ディフォルトはBASIC DATA文形式で出力)
  /n ---  セカンドオペレーション指定による最適化をOFFにする（ディフォルトはON）
  (/set) [シンボルラベル名]=[値/ラベル名] --- 任意のシンボルラベルを定義する。

/pオプションについては、■ 作成プログラムの実行 2.PBF形式 を参照下さい。
以下/nオプションについて説明します。このオプションを指定すると、$0,$30,$31を対象と
した転送命令のコード最適化を禁止し、PB-1000内蔵アセンブラと互換コードを出力します。
例）
/n オプション無し（ディフォルト）
02 42       LD $2,$0    ;セカンドオペレーション指定ON時 =2バイト命令 を出力
/n オプション付き
02 62 00    LD $2,$0    ;ディフォルト時（指定OFF）=３バイト命令を出力

PB-1000用のデータエリアのアドレスを決め打ちしている様なソースをアセンブルする場合に
ご利用下さい。尚、最適化される命令と出力コードの詳細は、添付のHD61700.sファイルを
アセンブルして出力されたlstファイルを参照下さい。
...
（適当な英語なのはご容赦を・・）
 ...
 オペランド記述ミス                  "Illigal Operand"
 START文が２つ以上ある               "START Already Defined"
 カンマ記述がおかしい                "Illigal [,]"
 ﾀﾞﾌﾞﾙｺｰﾃｰｼｮﾝ/括弧 異常              "Illigal ['']or[(]or[)]"
 ...
 実行アドレスが先頭ORG宣言より小さい "Execute Address Illigal"	
 演算異常( 0 除算等 )が発生          "Could not calculate"
 #if～#else～#endifのネストが異常    "Illigal [#if] - [#endif]"
 includeファイルがOPENできない       "Invalid Include File Name"
 includeネスト異常                   "Could Not Nest Include"
 メインレジスタ番号異常              "Illigal Register Number"
...
5. ｢KC-Disasembler｣ こたちゃん氏   ポケコンジャーナル1990/8
6. ｢AI-MON｣ こたちゃん氏   ポケコンジャーナル1990/8
7. ｢VX3 グラフィックサブルーチンパッケージ｣ こたちゃん氏 ポケコンジャーナル1990/8
8. ｢バイナリファイル←→テキスト変換プログラム｣ JUN AMANO 氏
   (｢CASIO PB-1000 Forever!｣ URL: http://www.lsigame.com/)
9. ｢mizuno HomePage｣ポケコンのお部屋 WWW board-2 のホーマー氏 すい氏の発言
10.「AI-アセンブラ」 P,H,M氏 ポケコンジャーナル 1992/12
11.「FX-870Pアセンブラ」陶治彩絵 氏 ポケコンジャーナル 1993/4
12.「X-Assembler Ver.6」N.Hayashi(はっくん)氏 ポケコンジャーナル 1995/2
